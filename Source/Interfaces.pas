...
//    function Types(Value: string): IModelGenericDataSet<T>;
  ...
  IModelTableOption = interface(IInterface)
    ...
    //1 Ponto inicial de pagina��o inicial
    function DisplayStart(Value: Integer): IModelTableOption;
    //1 Altere as op��es na selectlista de comprimento da p�gina .
    function LengthMenu(Value: string): IModelTableOption;
    //1 DOM Id
    function Name: string; overload;
    //1 DOM Id
    function Name(Value: string): IModelTableOption; overload;
    //1 Destrua qualquer tabela existente que corresponda ao seletor e substitua pelas novas op��es.
    function OpDestroy(Value: Boolean): IModelTableOption;
    //1 Ordem inicial(classifica��o)a ser aplicada � tabela
    function Order(Value: string): IModelTableOption;
    //1 Controlar em qual c�lula o manipulador de eventos do pedido ser� aplicado em uma coluna
    function OrderCellsTop(Value: Boolean): IModelTableOption;
    //1 Destaque as colunas que est�o sendo ordenadas no corpo da tabela
    function OrderClasses(Value: Boolean): IModelTableOption;
    //1 Ordena��o para sempre ser aplicada � tabela
    function OrderFixed(Value: string): IModelTableOption;
    //1 Controle de capacidade de ordena��o de v�rias colunas.
    function OrderMulti(Value: Boolean): IModelTableOption;
    //1 Alterar o comprimento da p�gina inicial(n�mero de linhas por p�gina)
    function PageLength(Value: Integer): IModelTableOption;
    //1 Op��es de exibi��o do bot�o de pagina��o
    function PagingType(Value: string): IModelTableOption;
    //1 Habilitar e configurar a extens�o responsiva para DataTables
    function Responsive(Value: Boolean): IModelTableOption;
    function Result: string;
    //1 Recuperar uma inst�ncia existente do DataTables
    function Retrieve(Value: Boolean): IModelTableOption;
    //1 Permita que a tabela reduza em altura quando um n�mero limitado de linhas for mostrado.
  ...
  IModelTableFeatures = interface(IInterface)
    ...
    //1 Controle diferenciado renderiza��o adiada para velocidade adicional de inicializa��o.
    function DeferRender(Value: Boolean): IModelTableFeatures;
    //1 Campo de exibi��o de informa��es da tabela de controle de recursos
    function Info(Value: Boolean): IModelTableFeatures;
    //1 O recurso controla a capacidade do usu�rio final de alterar o comprimento da exibi��o de pagina��o da tabela.
    function LengthChange(Value: Boolean): IModelTableFeatures;
    //1 Recursos de ordena��o de controle de recursos(classifica��o)no DataTables.
    function Ordering(Value: Boolean): IModelTableFeatures;
    //1 Ativar ou desativar a pagina��o da tabela
    ...
    //1 Economia de estado - restaura o estado da tabela no recarregamento da p�gina.
